...
WINDOW *printGameField(GameInfo_t gi) {
  ...
  for (int i = 0; i < FIELD_N; i++) {
    for (int j = 0; j < FIELD_M; j++) {
      if (gi.field[i][j] > 0) {  // Если клетка занята, рисуем блок.
        // Включаем цветовую пару для заполненных клеток.
        int color_figure = gi.field[i][j];
        wattron(game_window, COLOR_PAIR(color_figure));
        mvwprintw(game_window, i + 1, 3 * j + 1, "   ");  // Отрисовываем блок.
        wattroff(game_window, COLOR_PAIR(color_figure));  // Отключаем цвет.
      } else {
...
void printNextFigure(WINDOW *info_window, GameInfo_t gi) {
  ...
  for (int i = 0; i < gi.next_size; i++) {
    for (int j = 0; j < gi.next_size; j++) {
      if (gi.next[i][j] > 0) {  // Если элемент фигуры активен, рисуем его.
        int color_figure = gi.next[i][j];
        // Включаем цветовую пару для фигуры.
        wattron(info_window, COLOR_PAIR(color_figure));
        mvwprintw(info_window, i + 4, j * 3 + 6, "   ");  // Отрисовываем блок.
        wattroff(info_window,
                 COLOR_PAIR(color_figure));  // Отключаем цвет.
      }
    }
  }
}
